...
func (s *Session) InitGitHubClients() {
	for _, account := range s.Config.GitHubAccessTokens {
		ts := oauth2.StaticTokenSource(&oauth2.Token{AccessToken: account.Token})
		...
		if err != nil {
			if _, ok := err.(*github.ErrorResponse); ok {
				log.Error(fmt.Sprintf("Failed to validate token %s[..]: %s", account.Token[:10]), nil)
				continue
			}
		}

		s.Clients = append(s.Clients, &GitHubClientWrapper{client, account.Token, time.Now().Add(-1 * time.Second)})
	}

	if len(s.Clients) < 1 {
		log.Fatal("No valid GitHub tokens provided. Quitting!", nil)
	}
}
...
func (s *Session) GetClient() *GitHubClientWrapper {
	...
	log.Warn(fmt.Sprintf("All GitHub tokens exchausted/rate limited. Sleeping for %s", sleepTime.String()), nil)
